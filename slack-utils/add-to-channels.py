...
parser.add_argument("users", type=str, nargs="+", help="Slack User IDs")
parser.add_argument(
    "--channels",
    type=str,
    help="Which groups of channels to add the user to. Some special values are 'all' and 'managed', otherwise filters on prefix.",
)
...
if args.channels == "all":
    ...
    channels = [
        x
        for x in conversations["channels"]
        if x["is_archived"] is False and x["name"] in managed
    ]
else:
    channels = [
        x
        for x in conversations["channels"]
        if x["is_archived"] is False and x["name"].startswith(args.channels)
    ]
...
for channel in tqdm.tqdm(channels):
    ...
    try:
        app.client.conversations_invite(
            channel=channel["id"], users=",".join(args.users)
        )
